...
kernel ChebyshevNoise : ImageComputationKernel<ePixelWise>
{
    ...
    void process(int2 pos) {
        ...
        float3 input = float3(pos.x, pos.y, _z);
        ...
        for (int i = 0; i < _iterations; ++i) {
              angle = (float(i) * (_spread/float(_iterations))) + _rotation;
              rotated_input = rotateGradient(input, angle)* _scale / _size ;
              rotated_input = float3(rotated_input.x, rotated_input.y, (i*10)+_z);
